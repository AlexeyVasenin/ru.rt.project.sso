...
public class KeycloakOauth2UserService extends OidcUserService {
    ...
    private Collection<? extends GrantedAuthority> extractKeycloakAuthorities(OidcUserRequest userRequest) {
        ...
        if (checkAdminRealmAccess(token)) {
            JSONObject resourceAccess = token.getClaim("resource_access");
            if (resourceAccess.get("realm-management") != null) {
                JSONObject clientAccess = (JSONObject) resourceAccess.get("realm-management");
        ...
        return authoritiesMapper.mapAuthorities(new LinkedHashSet<>());
    }

    /**
     * Проверяет на наличие роли администратора реалма
     *
     * @param token токен
     * @return булевое значение наличия роли
     */
    private boolean checkAdminRealmAccess(Jwt token) {
        JSONObject realmAccess = token.getClaim("realm_access");
        JSONArray realmRoles = (JSONArray) realmAccess.get("roles");
        return realmRoles.contains("realm_admin");
    }
